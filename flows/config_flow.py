    ...
    def _analyze_handler_requirement(self):
        ...
        architect = Agent(
            ...
            backstory=(
                ...
                "- Mutations follow the action's function_type / call shape:\n"
                "  * insert: ctx.{table}.{action}(data={...})\n"
                "  * update: ctx.{table}.{action}(pk=..., data={...})\n"
                "  * delete: ctx.{table}.{action}(pk=...)\n"
                "  * bulk_insert: ctx.{table}.{action}(rows=[...])\n"
                "  * bulk_update: ctx.{table}.{action}(data={...}, conditions=[...])\n"
                "  * bulk_delete: ctx.{table}.{action}(conditions=[...])\n"
                "- Reads: ctx.{table}.get_by_pk(pk), .list(conditions=[...]), .count(...), .exists(...)\n"
                "- Raw SQL: ctx.raw_query(sql, params) for read-only SQL (JOINs, aggregations); params use $1,$2 syntax\n"
                "- Single-row actions return result['data'] = full row; bulk actions return result['data'] = {'count': N, 'pks': [...]}.\n"
                "- These are the ONLY ctx operations. No other methods exist.\n"
                "- All ctx calls share one BEGIN/COMMIT transaction (read-your-writes)\n\n"
                "PLATFORM AUTO-HANDLES (never do manually):\n"
                "- PK generation — read PKConfig to know whether insert data must include the PK:\n"
                "  * uuid4 / sequence / independent custom generator: omit PK\n"
                "  * custom generator that reads data['pk_field']: include PK in data\n"
                "- State injection and validation — NEVER pass 'state' in data={}, AND NEVER validate/check\n"
                "  state in handler logic. Action names encode the full transition (e.g.\n"
                "  set_X_approved_from_pending = verify pending → approved). CAS handles atomically.\n"
                "  The handler does NOT enforce transitions itself, but it MAY choose different actions\n"
                "  based on payload or business rules.\n"
                "- Type coercion — pass date/datetime/numeric/boolean as JSON strings directly\n"
                "- CAS for state transitions, transaction management, error translation\n\n"
                "IMPORTS FOR VALUE GENERATION (allowed and expected):\n"
                "When a handler step needs to GENERATE a value (timestamp, computed field),\n"
                "import the necessary library and produce the value as a STRING.\n"
                "Example: from datetime import datetime; datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%f')\n"
                "TypeCoercer converts the string to the correct DB type. This is different from\n"
                "type conversion (which TypeCoercer handles — never do manually).\n\n"
                "HANDLER CORE PATTERN — DATA MAPPING PIPELINE:\n"
                "A handler's primary job is to map business input into concrete action calls.\n"
                "For each step, identify both:\n"
                "  (1) the call shape: data / pk+data / pk / rows / data+conditions / conditions\n"
                "  (2) where each argument comes from: payload, previous results, or generated values only when needed\n"
                "Many steps are just payload → action, or previous output → next input, with no computation at all.\n"
                "The handler does NOT manage state.\n\n"
                "PAYLOAD CONTRACT:\n"
                "- Unless the user explicitly requests nesting, payload remains FLAT.\n"
                ...
                "- Handler validation is for payload/business rules, not for reproducing DB schema validation.\n"
                "- Use schema knowledge to understand mapping, likely required inputs, and action arguments.\n"
                "- Do NOT add scattered per-column checks for every mapped DB/source field unless the user\n"
                "  explicitly wants a custom business validation error.\n\n"
                "BUSINESS-DRIVEN DESIGN THINKING:\n"
                "Before designing, use dp_file_read(category='tables') to read each involved table's\n"
                "config. The purpose is to DISCOVER available actions and SELECT the right ones for\n"
                "the user's business flow. From the config, extract:\n"
                "- PK field + PKConfig strategy: determines whether caller supplies PK or platform generates it.\n"
                "  * uuid4/sequence: auto-generated — handler does NOT pass PK, reads it from result.\n"
                "  * custom generator that reads the PK from data: caller MUST supply PK in data\n"
                "    (typical for child tables where PK = FK from parent table).\n"
                "  * custom with independent generator: auto-generated — handler reads PK from result.\n"
                "- Columns (ColumnDef): pg_type, nullable, check constraints, default_expr, unique —\n"
                "  this tells you what the payload and action arguments likely need.\n"
                "  Required caller input is usually NOT NULL without default_expr, unless supplied by\n"
                "  previous steps or generated values.\n"
                "- FK definitions: which columns reference other tables — this determines step order\n"
                "  (parent tables must be created first so the PK can be passed to child rows).\n"
                "- Action function_type / call shape: tells you whether the step needs data, pk+data,\n"
                "  pk, rows, data+conditions, or conditions.\n"
                "- State machine (states, transitions): which actions trigger which state changes.\n"
                "  The action name encodes the transition (e.g., create_order_active = insert + set active).\n"
                "- Registered actions: discover what actions exist and SELECT the ones matching\n"
                "  the user's required business flow. Action names encode state transitions\n"
                "  (e.g. create_X_active, set_X_approved_from_pending). Use ONLY names from the config.\n"
                "Use this knowledge to design the handler's step order, exact call signature per step,\n"
                "payload shape, and error strategy.\n\n"
                "ERROR HANDLING:\n"
                "- raise HandlerError(message=..., code=..., http_status=...) for payload/business-rule errors\n"
                ...
                "- User gave payload fields → extract directly and preserve names exactly\n"
                "- Unless user explicitly asks for nesting, keep payload flat\n"
                ...
                "- payload_fields: what the caller sends (name, type, required); flat by default unless user explicitly requested nesting\n"
        ...
        prompt += (
            ...
            "STEP 1 — READ TABLE CONFIGS (do this FIRST):\n"
            "  - Identify tables mentioned → use dp_name_resolve to verify they exist\n"
            "  - For EACH verified table, call dp_file_read(category='tables', filename='{table}.py')\n"
            "  - From each config, note: PK field + strategy, column definitions (pg_type, nullable,\n"
            "    constraints), registered action names, function_type/call shape, state machine,\n"
            "    FK references to other tables.\n"
            "  - This is essential — you cannot design a correct handler without knowing\n"
            "    the exact action names, call signatures, column types, and FK relationships.\n\n"
            "STEP 2 — EXTRACT what user provided:\n"
            "  - Handler name (or derive from the purpose)\n"
            "  - Which actions the user wants to invoke (match against table config action names)\n"
            "  - Exact payload fields the user described — preserve names exactly\n"
            "  - If user did not ask for nested structure, assume flat payload\n\n"
            "STEP 3 — BUSINESS REASONING (think through the design):\n"
            "  - Action selection: read table configs to discover available actions. Select actions\n"
            "    whose state transitions match the user's requirements. Action names encode transitions.\n"
            "  - Call shape: for each selected action, determine whether the actual call is\n"
            "    data / pk+data / pk / rows / data+conditions / conditions.\n"
            "  - Step order: parent tables first (so PK flows to FK-dependent child rows)\n"
            "  - Data mapping: for each step, identify where each argument comes from:\n"
            "    payload, previous action result, or generated value only when needed.\n"
            "  - Generated values: identify fields the handler must generate at runtime (timestamps,\n"
            "    computed fields). These are generated as STRINGS → TypeCoercer converts to DB types.\n"
            "  - Payload shape: keep payload flat unless user explicitly asked for nesting.\n"
            "    Preserve user field names exactly; map them to action arguments / DB fields as needed.\n"
            "    Use schema to infer likely required caller inputs, not to rename fields.\n"
            "  - State: fully managed by actions via CAS — handler NEVER checks, validates, or sets state.\n"
            "    Action name encodes the transition (e.g. set_X_approved_from_pending).\n"
            "    The handler may still choose different actions based on payload/business rules.\n"
            "  - PK handling: depends on PKConfig strategy from table config.\n"
            "    uuid4/sequence/independent custom → PK auto-generated, extract from result[\"data\"][\"pk_field\"]\n"
            "    custom generator that reads the PK from data → caller MUST supply PK in data (e.g. FK from parent)\n"
            "  - Return shape: single-row actions return full row in result['data']; bulk actions return count+pks only.\n"
            "  - Validation: only add payload/business-rule checks the flow actually needs.\n"
            "    Do NOT replicate DB/action schema validation for every mapped field.\n\n"
            ...
            "STEP 5 — When ALL decisions are available, produce handler_design:\n"
            ...
            "  - payload_fields: each with name, field_type, required; flat by default unless user explicitly requested nesting\n"
            ...
            "    input_mapping must describe the exact call signature, e.g. pk=..., data={...}\n"
            "    or rows=[...], or data={...}, conditions=[...].\n"
            "    output_key should describe what is carried forward: full row, extracted PK,\n"
            "    or bulk pks/count.\n"
    ...
    def _revise_handler_design(self, feedback: str) -> dict[str, Any]:
        ...
        reviser = Agent(
            ...
            backstory=(
                ...
                "- input_mapping must preserve the exact call shape: pk=..., data={...}, rows=[...], or conditions=[...]\n"
                "- Preserve user-provided payload field names unless the user explicitly requests renaming\n"
                "- Keep payload flat unless the user explicitly requests nesting\n"
    ...
    def _generate_handler_design_fallback(self) -> HandlerDesign | None:
        ...
        builder = Agent(
            ...
            backstory=(
                ...
                "BEFORE DESIGNING: use dp_file_read(category='tables') to read each table's config.\n"
                "The purpose is to DISCOVER available actions and SELECT the right ones for the user's\n"
                "business flow. From the config, learn action names (which encode state transitions),\n"
                "function_type/call shape, PK field, column types/constraints, FK relationships.\n"
                "Use ONLY real action names.\n\n"
                "DESIGN PRINCIPLES:\n"
                "- Action selection: read config to find actions matching user's state transitions.\n"
                "  Action names encode transitions (e.g. create_X_active, set_X_approved_from_pending).\n"
                "- Step order follows FK dependencies (parent before child)\n"
                "- Call shape: determine whether each step is data / pk+data / pk / rows / data+conditions / conditions\n"
                "- Data mapping: each argument comes from payload, previous results, or generated values only when needed\n"
                "- Generated values: timestamps, computed fields — generate as STRINGS, TypeCoercer converts\n"
                "- PK handling depends on PKConfig: uuid4/sequence/independent-custom → auto-generated,\n"
                "  extract from result; custom generator that reads the PK from data → caller must supply in data\n"
                "- State is fully managed by actions — NEVER include 'state' in data, NEVER validate state\n"
                "- Return shape: single-row actions return full row; bulk actions return count+pks only\n"
                "- Payload stays flat unless user explicitly requests nesting\n"
                "- Preserve user-provided payload field names exactly; do not invent aliases\n"
                "- Use schema to understand likely required inputs and mapping, not to add scattered schema validation\n\n"
                "RULES:\n"
                "- handler_name: snake_case\n"
                "- mode: 'sync' (default) or 'async'\n"
