    ...
    def _analyze_handler_requirement(self):
        ...
        architect = Agent(
            ...
            backstory=(
                ...
                "- PK generation — but whether to include PK in data={} depends on PKConfig strategy:\n"
                "  * uuid4/sequence: auto-generated, do NOT include PK in data\n"
                "  * custom with generator=lambda data: data[\"pk_field\"]: MUST include PK in data\n"
                "    (e.g., child table whose PK = FK from parent)\n"
                "  * custom with independent generator: auto-generated, do NOT include PK in data\n"
                "  → Always read the table config to determine which case applies.\n"
                "- State injection — NEVER pass 'state' in data={}, the platform sets it from transitions\n"
                ...
                "config. From the config, extract:\n"
                ...
                "  * custom with generator=lambda data: data[\"pk\"]: caller MUST supply PK in data\n"
                ...
                "- Registered actions: use ONLY action names that exist in the config. Do NOT invent\n"
                "  generic names like 'create', 'update', 'delete'.\n"
                "Use this knowledge to design the handler's step order, payload shape, and error strategy.\n\n"
                "ERROR HANDLING:\n"
        ...
        prompt += (
            ...
            "    constraints), registered action names, state machine, FK references to other tables.\n"
            "  - This is essential — you cannot design a correct handler without knowing\n"
            "    the exact action names, column types, and FK relationships.\n\n"
            ...
            "  - State handling: the platform auto-manages state — never include 'state' in data dicts\n"
            ...
            "    custom with lambda data: data[\"pk\"] → caller MUST supply PK in data (e.g. FK from parent)\n"
    ...
    def _generate_handler_design_fallback(self) -> HandlerDesign | None:
        ...
        builder = Agent(
            ...
            backstory=(
                ...
                "From the config, learn the exact action names, PK field, column types/constraints,\n"
                "FK relationships, and state machine. Use ONLY real action names from the config.\n\n"
                "DESIGN PRINCIPLES:\n"
                ...
                "  extract from result; custom with lambda data: data[\"pk\"] → caller must supply in data\n"
                "- State is auto-injected — NEVER include 'state' in data dicts\n"
